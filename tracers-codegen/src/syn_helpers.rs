...
/// In the `tracers` macro implementation there are some cases where, given a `Type` instance, I
...
pub(super) fn transform_types<F: FnMut(&syn::Type) -> TracersResult<syn::Type>>(
    typ: &syn::Type,
    mut f: F,
...
fn recurse_tree<F: FnMut(&syn::Type) -> TracersResult<syn::Type>>(
    f: &mut F,
    typ: &syn::Type,
) -> TracersResult<syn::Type> {
...
fn recurse_path<F: FnMut(&syn::Type) -> TracersResult<syn::Type>>(
    f: &mut F,
    path: &syn::Path,
) -> TracersResult<syn::Path> {
