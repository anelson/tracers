...
impl ProbeCallSpecification {
    pub fn from_token_stream(tokens: TokenStream) -> TracersResult<ProbeCallSpecification> {
        ...
        match syn::parse2::<syn::Expr>(tokens) {
            ...
            Err(e) => Err(TracersError::syn_error(
...
impl ProbeCallDetails {
    ...
    pub fn from_call_expression(call: syn::Expr) -> TracersResult<ProbeCallDetails> {
        match call {
            syn::Expr::Call(call) => {
                ...
                if let syn::Expr::Path(func) = func {
                    if func.path.segments.len() < 2 {
                        return Err(
                            TracersError::invalid_call_expression(format!(
            ...
            _ => {
                Err(TracersError::invalid_call_expression( "The probe! macro requires the name of a provider trait and its probe method, e.g. MyProvider::myprobe(...)", call))
            }
        }
    }
}
