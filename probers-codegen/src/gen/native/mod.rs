...
//! native C support for user-mode tracing used by DTrace, SystemTap USDT, and DTrace USDT.  This
...
//! enabled, which would not be incurred by equivalent native code.  But otherwise it's identical
//! to the native code implementation.  In limited cases when cross-language LTO is possible, the
//! optimizer can even remove this method call and produce the same performance as native C code.
//!
